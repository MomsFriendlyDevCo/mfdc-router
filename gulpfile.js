...
gulp.task('build', ['build:mfdc-router', 'build:angular-mfdc-router', 'build:angular-mfdc-router-debug']);
...
gulp.task('build:angular-mfdc-router-debug', function () {
	gulp.src('./src/angular-mfdc-router.js')
		.pipe(rename('angular-mfdc-router-debug.js'))
		.pipe(preprocess({
			includeBase: __dirname,
			context: {angular: true, debug: true},
		}))
		.pipe(replace(/^.*require\(.+?\);\s*$/gm, ''))
		.pipe(replace(/^.*module\.exports = .*$/gm, ''))
		.pipe(replace(/\n{3,}/g, '\n'))
		...
		.pipe(replace(/^'use strict';$/gm, ''))
		.pipe(gulp.dest('./dist'))
		.pipe(rename('angular-mfdc-router-debug.min.js'))
		.pipe(uglify({mangle: false}))
		.pipe(gulp.dest('./dist'));
});
